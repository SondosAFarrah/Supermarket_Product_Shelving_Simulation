...
int item_count, team_count, shm_id3 , shm_id2, shm_id1;
...
int main() {
    ...
    for (int i = 0; i < teamsNum; i++) {
        ...
        if (pid == 0) {
           ...
           // teamArea[i].msgQueueId = msgget(IPC_PRIVATE, IPC_CREAT | 0666);
            ...
            if(checkPid == 0 ){
            char id2[10]; 
             sprintf(id2,"%d", (int)shm_id2); 
                char id1[10]; 
              sprintf(id1,"%d", (int)shm_id1);
               execlp("./storageguard", "storageguard",id1, id2, NULL);
             //  execlp("./storageguard", "storageguard",shm_id1, shm_id2, NULL);
               exit(EXIT_SUCCESS);
             }
            else if (checkPid <0){
                exit(EXIT_FAILURE);
    ...
    Product *storageSharedMemory() {
        ...
        // Allocate shared memory segment
         shm_id1 = shmget(key, SHM_SIZE, IPC_CREAT | 0666);
        // cashiersSMID = shm_id;
        if (shm_id1 == -1) {
            perror("shmget");
            exit(-1);
    ...
    Product *shelveSharedMemory() {
        ...
        // Allocate shared memory segment
         shm_id2 = shmget(key1, SHM_SIZE, IPC_CREAT | 0666);
    ...
    Team *teamsSharedMemory() {
        ...
        // Allocate shared memory segment
         shm_id3 = shmget(key, SHM_SIZE, IPC_CREAT | 0666);
